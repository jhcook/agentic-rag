...
**Agentic RAG** is a retrieval-augmented generation system supporting multiple LLM backends (Ollama local/cloud, OpenAI Assistants, Google Vertex AI). It combines FAISS vector search, document indexing, and LangChain extractors with FastAPI REST + MCP servers and a React/Vite UI.
...
2. **Document Store & Index** ([src/core/rag_core.py](src/core/rag_core.py), [src/core/store.py](src/core/store.py))
   - `Store` class: JSONL document storage at [cache/rag_store.jsonl](cache/rag_store.jsonl)
   - FAISS index: built from sentence-transformers embeddings ([src/core/embeddings.py](src/core/embeddings.py))
   - Index rebuild: `rebuild_index()` with threading lock to prevent concurrent rebuilds
...
- Verify FAISS index rebuild: `curl -X POST http://localhost:8001/api/rebuild-index`

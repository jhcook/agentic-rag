...
- If asked to index local files (e.g., "index docs", "reindex documents"), call `retrieval-server.index_documents_tool` with the `path` argument (e.g., `path="docs"`).
- If asked to index a URL (e.g., from http/https), call `retrieval-server.index_url_tool` with the `uri` argument.
